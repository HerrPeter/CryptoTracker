...
void MySQL_CC_Setup(sql::Statement *stmt) {
  ...
  stmt->execute("CREATE TABLE UserCoinID ( coin_id int UNSIGNED NOT NULL AUTO_INCREMENT, coin_symbol varchar(5), u_name varchar(20) NOT NULL, add_date int(10) UNSIGNED, amount DOUBLE PRECISION (12,2), last_price DOUBLE PRECISION(10,2), add_price DOUBLE PRECISION(10,2), PRIMARY KEY(coin_id), INDEX(coin_symbol), INDEX(u_name), FOREIGN KEY (coin_symbol)  REFERENCES CryptoCoin(symbol), FOREIGN KEY (u_name) REFERENCES UserData(username))");
}
